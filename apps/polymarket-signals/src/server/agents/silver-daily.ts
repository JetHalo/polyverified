...
export const silverDailyAgent: SignalAgent = {
  slug: "silver-daily",
  marketType: "Silver Daily",
  displayName: "Silver Daily Agent",
  async shouldRun(input) {
    return passesDailyExecutionWindow(input)
      && passesMarketQualityFilters(input, {
        minLiquidityUsd: input.config.strategy.daily.silverMinLiquidityUsd,
        maxSpreadBps: input.config.strategy.daily.silverMaxSpreadBps,
      });
  },
